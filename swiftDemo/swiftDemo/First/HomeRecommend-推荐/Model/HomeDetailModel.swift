...
//播放列表详情页数据模型
struct HomeDetailModel: HandyJSON {
    var album:HeaderIntroductionModel?
    var user:AnchorInfoModel?
    var tracks:ProgramListModel?
...
//头部以及内容简介信息
struct HeaderIntroductionModel:HandyJSON {
    var albumId:Int = 0
...
//主播信息模型
struct AnchorInfoModel:HandyJSON {
    var uid:Int = 0
    var nickname:String?
    var isVerified:Bool = false
...
//节目列表模型
struct ProgramListModel:HandyJSON {
    var list:[ProgramDetailModel]?
}

//节目详情模型
struct ProgramDetailModel:HandyJSON {
    var trackId:Int = 0
    var uid:Int = 0
    var duration:Int = 0
    var albumId:Int = 0
