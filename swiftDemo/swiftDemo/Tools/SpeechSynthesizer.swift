...
//        if let error = try? session.setCategory(AVAudioSession.Category.playback, with: AVAudioSession.CategoryOptions.duckOthers) {
//            NSLog("AudioSession Error:\(error)")
//        }
        if let error = try? session.setCategory(AVAudioSession.Category.playback, mode: AVAudioSession.Mode.default, options: AVAudioSession.CategoryOptions.duckOthers) {
            NSLog("AudioSession Error:\(error)")
        }
